...
namespace Server.Spells.Druid
{
    public class HurricaneSpell : DruidicSpell
    {
        ...
        public override string SpellDescription
        {
            get
            {
                return "Calls forth a violent hurricane that damages any enemies within range.";
            }
        }
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                if (Core.AOS)
                    duration = TimeSpan.FromSeconds((12 + (Caster.Skills.AnimalLore.Fixed / 5)) / 4);
                else
                    duration = TimeSpan.FromSeconds(3.0 + (Caster.Skills[SkillName.AnimalLore].Value * 0.5));
        ...
        private class InternalItem : Item
        {
            ...
            public override bool OnMoveOver(Mobile m)
            {
                if (Visible && m_Caster != null && (!Core.AOS || m != m_Caster) && SpellHelper.ValidIndirectTarget(m_Caster, m) && m_Caster.CanBeHarmful(m, false))
                {
                    ...
                    if (!Core.AOS && m.CheckSkill(SkillName.MagicResist, 0.0, 100.0))
                    {
                        damage = 1;
