	...
	public class ControlCommand
	{
		...
		private static void ChangeControl( Mobile target, ControlItem controlItem, bool stats, bool skills, bool items )
		{
			...
			if ( oldNPC != null )
			{
				...
				{
					from.SendMessage("Le NPC d'origine a été supprimé et ne sera pas restauré. La raison aurait pu être une réapparition manuelle");
					oldNPC.Delete();
				}
			}
			...
			{
				from.SendMessage("Vous prenez le relais {0}, {1}", target.Name, target.Title);
				//"You Control  {0}, {1}"
